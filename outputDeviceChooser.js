/*
 * Output device QuickSettings indicator.
 * SystemIndicator + QuickMenuToggle for selecting audio output devices.
 *
...
import {OutputDeviceChooser} from './deviceChooserBase.js';
import * as Port from './portSettings.js';

const OutputDeviceToggle = GObject.registerClass(
class OutputDeviceToggle extends QuickSettings.QuickMenuToggle {
    _init() {
        super._init({
            title: 'Output',
...
export const OutputDeviceIndicator = GObject.registerClass(
class OutputDeviceIndicator extends QuickSettings.SystemIndicator {
    _init(settings, extensionDir, gettext) {
        super._init();
        this._settings = settings;
        this._ = gettext;

        this._toggle = new OutputDeviceToggle();
        this._toggle.menu.setHeader('audio-card-symbolic', gettext('Output Devices'));
        this.quickSettingsItems.push(this._toggle);

        this._chooser = new OutputDeviceChooser(settings, extensionDir, gettext);
        ...
        this._settingsSignals.push(
            this._settings.connect(`changed::${Port.SHOW_OUTPUT_DEVICES}`,
    ...
    _updateVisibility() {
        let enabled = this._settings.get_boolean(Port.SHOW_OUTPUT_DEVICES);
    ...
    _updateActiveDevice(device) {
        ...
        if (iconName) {
            ...
            this._toggle.menu.setHeader(iconName, this._('Output Devices'));
        }
    }
    ...
    _rebuildMenu() {
        ...
        for (let device of devices) {
            let iconName = this._chooser.getIcon(device.icon_name) || 'audio-card-symbolic';
